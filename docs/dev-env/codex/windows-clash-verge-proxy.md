...
4. 在当前 Windows + Codex 桌面版环境里，`HTTP_PROXY` 和 `HTTPS_PROXY` 是关键项
5. `ALL_PROXY` 不默认推荐，它可能扩大影响面，干扰其他程序
6. 如果代理已经生效但仍频繁重连，优先怀疑节点质量，不要先怀疑规则
...
- `HTTP_PROXY` 应为 `http://127.0.0.1:7897`
- `HTTPS_PROXY` 应为 `http://127.0.0.1:7897`
- `ALL_PROXY` 应为空

如果前两个为空，说明环境变量没有写进去；如果 `ALL_PROXY` 仍有值，说明还没清理干净。
...
### 情况 0：系统代理开启，但 Codex 一删环境变量就开始 `Reconnecting...`

这说明在当前环境里：

- Codex 不能只靠 Clash 系统代理稳定工作
- 至少还需要 `HTTP_PROXY` 和 `HTTPS_PROXY`
- 这也是为什么本文推荐“最小环境变量配置”，而不是只写“打开系统代理”
...
### 情况 0.5：Codex 恢复了，但其他程序开始变慢或异常

这次实际排查里出现过一个很典型的现象：

- 只开 Clash 系统代理时，某些其他程序原本正常
- 加上 `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY` 后，Codex 稳定了
- 但删掉全部环境变量后，Codex 又开始重连
- 最后收敛到只保留 `HTTP_PROXY` / `HTTPS_PROXY`，移除 `ALL_PROXY`，Codex 正常，副作用更小

这说明：

- `ALL_PROXY` 不是当前 Codex 稳定运行的必要条件
- 但它可能扩大代理影响范围，波及其他程序

因此本文不再推荐默认配置 `ALL_PROXY`。
